...
World::World(commons::Params* params)
    : commons::BaseType(params),
      ...
      agents_(),
      world_time_(0.0),
      remove_agents_(params->get_bool(
          "World::remove_agents_out_of_map",
          "Whether agents should be removed outside the bounding box.", false)),
      calculate_driving_corridor_(params->get_bool(
          "World::update_horizon_corridor",
          "Whether the horizon corridor should be updated.", true)),
      driving_corridor_length_(
          params->get_real("World::driving_corridor_length",
                           "Length of driving corridor.", 40.)),
      recalculate_driving_corridor_(params->get_bool(
          "World::recalculate_driving_corridor",
          "Whether the driving corridor should be recalculated.", true)) {}
...
void World::DoPlanning(const float& delta_time) {
  UpdateAgentRTree();
  if (calculate_driving_corridor_) UpdateHorizonDrivingCorridors();
...
void World::DoExecution(const float& delta_time) {
  ...
  if (recalculate_driving_corridor_) RecalculateDrivingCorridors();
}
...
void World::remove_agent(const objects::AgentPtr& agent) {
  agents_.erase(agent->agent_id_);
  UpdateAgentRTree();
}
...
AgentMap World::GetAgentsIntersectingPolygon(
  ...
  for (auto& result_pair : query_results) {
    ...
    if (modules::geometry::Collide(
            agent->GetPolygonFromState(agent->get_current_state()), polygon)) {
